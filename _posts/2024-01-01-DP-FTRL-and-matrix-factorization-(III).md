---
comments: true
title: DP-FTRL and matrix factorization mechanism (III)
...
where in the last step we use the bounded norm of gradients. Combining the above with [(3.1)](#eq:max-error) and the fact $$\sigma = \sigma_{\epsilon,\delta} \cdot \mathrm{sens}(\bC)$$ together, we conclude that for $$(\epsilon,\delta)$$-DP under $$(\bB,\bC)$$ for the task of prefix sum, the max-error across all $$T$$ noisy prefix sum is upper bounded by the following with high probability

<a id="eq:max-error-final"></a>
$$
...
where one can think of $$\ex{\norm{\bB \bZ}}_F$$ as the total expected error in all $$T$$ outputs and hence normalized by $$1/\sqrt{T}$$ to have the same unit as the maximal expected error. In most existing works, the bound for [(3.3)]($ex-error) is used directly in the regret of DP-FTRL (i.e., [(1.2)](https://xingyuzhou.org/blog/notes/DP-FTRL-and-matrix-factorization-(I)#eq2)). However, in this case, one can at most get an expected regret and hence an expected population excess risk. Let us first put this aside and focus on how to bound the term in [(3.3)]($ex-error).
